...
local prettytext = include("autorun/client/prettytext.lua")
...
local player_line = {
	Init = function(self)
		...
		self.btn.OnMousePressed = function(self,num) -- this part is highly trash
			local isulx = _G.ulx ~= nil
			local prefix = isulx and "ulx" or "aowl"
			local PlayerID = not isulx and tostring(parent.Player:UniqueID()) or "$" .. parent.Player:SteamID()
			...
			if num == MOUSE_RIGHT then
				...
				if _G.aowl or _G.ulx then
					if PlayerID ~= tostring( LocalPlayer():UniqueID() ) then
						local goto_ = self.Menu:AddOption("Goto", function()
							RunConsoleCommand(prefix, "goto", PlayerID )
						end)

						goto_:SetImage("icon16/arrow_right.png")

						local bring = goto_:AddSubMenu( "Bring" )
						bring:AddOption("Bring",function() RunConsoleCommand(prefix, "bring", PlayerID  ) end):SetImage("icon16/arrow_in.png")
					end

					local SubAdmin,pic = self.Menu:AddSubMenu("Staff")
					pic:SetImage("icon16/shield.png")
					SubAdmin:AddOption( "Kick",function() cinputs(prefix .. " kick "..PlayerID  , 1) end):SetImage("icon16/door_in.png")
					SubAdmin:AddOption( "Ban",function() cinputs(prefix .. " ban "..PlayerID  , 2) end):SetImage("icon16/stop.png")
					SubAdmin:AddOption( "Cleanup",function() RunConsoleCommand(prefix, "cleanup", PlayerID) end):SetImage("icon16/arrow_rotate_clockwise.png")
					SubAdmin:AddSpacer()
					SubAdmin:AddOption( "Reconnect",function() RunConsoleCommand(prefix, "cexec", PlayerID , "retry") end):SetImage("icon16/arrow_refresh.png")
			...
			elseif num == MOUSE_LEFT then
                ...
                if selected_player:IsBot() then
                    ply_lines[selected_player:Nick()].Selected = false
                else
		...
		self.btn.Paint = function(self,w,h)
			...
			prettytext.DrawText({
				text = team.GetName(parent.Player:Team()):upper(),
				x = w*3/scale_coef,
				y = 5,
				font = "Square721 BT",
				size = 18,
		 	...
		 	local time = GetPlayerTime(parent.Player)
	...
	Setup = function(self,ply)
		...
		self.SteamID = ply:IsBot() and ply:Nick() or ply:SteamID()
	end,
}
...
local scoreboard = {
	Init = function(self)
		...
		lvlsort.Paint = sort_paint("RANK")
		lvlsort.DoClick = sort_rows(function(v) return v.Player:Team() end)
		...
		players.Think = function(self)
			for k,v in pairs(player.GetAll()) do
                local add = false
                if v:IsBot() and not ply_lines[v:Nick()] then add = true end
                if not v:IsBot() and not ply_lines[v:SteamID()] then add = true end
				if add then
					local line = self:Add("ScoreboardPlayerLine")
					line:Dock(TOP)
					line:Setup(v)
                    if v:IsBot() then
                        ply_lines[v:Nick()] = line
                    else
					    ply_lines[v:SteamID()] = line
                    end
				end

				ply_lines[v:IsBot() and v:Nick() or v:SteamID()].Color = team.GetColor(v:Team())
			end

			if IsValid(selected_player) then
                if selected_player:IsBot() then
                    if not ply_lines[selected_player:Nick()].Selected then
                        ply_lines[selected_player:Nick()].Selected = true
                    end
                else
                    if not ply_lines[selected_player:SteamID()].Selected then
...
do
	...
	local status = {
		...
		Paint = function(self,w,h)
			...
			local text_x,text_y = spacing,spacing
			prettytext.DrawText({
				text = str,
				size = 16,
				blur_size = 4,
